...
class ScreenCapture:
    ...
    def start_capture(self, interval_seconds: float = 1.0, quality: int = 25, fps: int = 1):
        if self._running:
        ...
        with mss.mss() as sct:
            while self._running:
                try:
                    ...
                    self._latest_frame = byte_arr.getvalue()

                except Exception as e:
                    logger.error(f"Ошибка при захвате экрана: {e}")
                    self._latest_frame = None  # Сбрасываем кадр при ошибке
